...
import type { loginCredentials, onSuccessResponseType, registerCredentials, UserType } from "../../utils/types";
...
class Authentication{
	...
	async login(credentials: loginCredentials) {
		...
		try {
			const response: onSuccessResponseType = await httpAxios.post("auth/login/", credentials);
	...
	async register(credentials: registerCredentials) {
		// Request for getting an access token to user with his credentials.
		try {
			const response: onSuccessResponseType = await httpAxios.post("auth/signup/", credentials);			
            onSuccessResponse(response);
			const tokensType: TokensType = {"access_token": response.data.access_token, "refresh_token": response.data.refresh_token}
			return tokensType;
		} catch (error) {
	...
	async invitation(signature: string){
		// Request to register new user that came with an invitation from email, must include a signature to continue the process.
		try {
			const response: onSuccessResponseType = await httpAxios.get(`auth/invitation/?signature=${signature}`, {});			
            onSuccessResponse(response);
